...
public class SocketGameClient {
    ...
    private void doConnect(String host, int port, String playerName) {
        ...
        try {
            ...
            // Send handshake immediately; do not enqueue to the read-loop executor.
    ...
    private void handleServerMessage(String message) {
        ...
        switch (type) {
            ...
            case "MATCHED":
                ...
                if (parts.length >= 2) {
                    listener.onMatched(parseInt(parts[1], 0));
                }
                break;
            case "STATE":
                // 房间实时状态：双方分数和死亡状态。
            ...
            case "SETTLE":
                // 双方都死亡后，服务器广播最终结算分数。
                if (parts.length >= 3) {
                    listener.onSettle(parseInt(parts[1], 0), parseInt(parts[2], 0));
                }
                break;
            case "OPPONENT_LEFT":
                // 对手断开连接或主动退出。
                if (parts.length >= 2) {
                    listener.onOpponentLeft(parseInt(parts[1], 0));
                }
                break;
            case "ERROR":
