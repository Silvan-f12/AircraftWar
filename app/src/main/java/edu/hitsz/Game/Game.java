...
public abstract class Game {
    ...
    public interface NetworkSyncListener {
        void onLocalScoreChanged(int score);

        void onLocalDead(int finalScore);
    }
    ...
    private NetworkSyncListener networkSyncListener;
    private int lastSyncedScore = -1;
    private int networkPlayerIndex = 1;
    private int opponentScore = 0;
    private boolean opponentDead = false;
    private boolean localDeadNotified = false;
    private String networkStatus = "单机模式";
    private boolean networkMatchReady = true;
...
//            }
        ...
        if (networkSyncListener != null && score != lastSyncedScore) {
            lastSyncedScore = score;
            networkSyncListener.onLocalScoreChanged(score);
        }

        if (heroAircraft.getHp() <= 0) {
            if (!localDeadNotified && networkSyncListener != null) {
                localDeadNotified = true;
                networkSyncListener.onLocalDead(score);
            }
    ...
    public void draw(Canvas canvas) {
        ...
        float hudOppY = hudLifeY + screenHeight * HUD_LINE_GAP_RATIO;
        float hudNetY = hudOppY + screenHeight * HUD_LINE_GAP_RATIO;
        canvas.drawText("SCORE: " + score, hudX, hudScoreY, scorePaint);
        canvas.drawText("LIFE: " + heroAircraft.getHp(), hudX, hudLifeY, scorePaint);
        canvas.drawText("OPPONENT: " + opponentScore + (opponentDead ? " (DEAD)" : ""), hudX, hudOppY, scorePaint);
        canvas.drawText("NET: " + networkStatus, hudX, hudNetY, scorePaint);
    ...
    public void setNetworkSyncListener(NetworkSyncListener networkSyncListener) {
        this.networkSyncListener = networkSyncListener;
    }

    public void setNetworkPlayerIndex(int networkPlayerIndex) {
        this.networkPlayerIndex = networkPlayerIndex;
    }

    public int getNetworkPlayerIndex() {
        return networkPlayerIndex;
    }
    ...
    public void setNetworkMatchReady(boolean networkMatchReady) {
        this.networkMatchReady = networkMatchReady;
    }

    public void updateOpponentState(int opponentScore, boolean opponentDead) {
        this.opponentScore = opponentScore;
        this.opponentDead = opponentDead;
    }

    public void applyNetworkSettlement(int localFinalScore, int opponentFinalScore) {
        this.score = localFinalScore;
        this.lastSyncedScore = localFinalScore;
        this.opponentScore = opponentFinalScore;
