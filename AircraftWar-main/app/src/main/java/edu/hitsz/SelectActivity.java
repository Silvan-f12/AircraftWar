...
public class SelectActivity extends AppCompatActivity {
    ...
    private void initDifficultySection() {
        ...
        LinearLayout onlineSwitchRow = new LinearLayout(this);
        onlineSwitchRow.setOrientation(LinearLayout.HORIZONTAL);
        onlineSwitchRow.setGravity(android.view.Gravity.CENTER_VERTICAL);
        onlineSwitchRow.setPadding(dp(20), dp(4), dp(20), dp(4));
        onlineModeCheck = new CheckBox(this);
        onlineModeCheck.setText("联机模式");
        onlineModeCheck.setChecked(isOnlineMode);
        onlineModeCheck.setOnCheckedChangeListener((buttonView, checked) -> {
            isOnlineMode = checked;
            if (hostInput != null) {
                hostInput.setEnabled(checked);
            }
            if (portInput != null) {
                portInput.setEnabled(checked);
            }
        });
        onlineSwitchRow.addView(onlineModeCheck);
        buttonContainer.addView(onlineSwitchRow);

        LinearLayout hostRow = new LinearLayout(this);
        hostRow.setOrientation(LinearLayout.HORIZONTAL);
        hostRow.setGravity(android.view.Gravity.CENTER_VERTICAL);
        hostRow.setPadding(dp(20), dp(4), dp(20), dp(4));

        TextView hostLabel = new TextView(this);
        hostLabel.setText("服务器: ");
        hostLabel.setTextSize(16);

        hostInput = new EditText(this);
        hostInput.setText(serverHost);
        hostInput.setHint("10.0.2.2");
        hostInput.setEnabled(isOnlineMode);
        LinearLayout.LayoutParams hostInputParams = new LinearLayout.LayoutParams(
                0, LinearLayout.LayoutParams.WRAP_CONTENT, 1f);
        hostInput.setLayoutParams(hostInputParams);
        hostRow.addView(hostLabel);
        hostRow.addView(hostInput);
        buttonContainer.addView(hostRow);
        ...
        portRow.setPadding(dp(20), dp(4), dp(20), dp(10));
        ...
        portInput.setHint("7777");
        portInput.setEnabled(isOnlineMode);
        LinearLayout.LayoutParams portInputParams = new LinearLayout.LayoutParams(
                0, LinearLayout.LayoutParams.WRAP_CONTENT, 1f);
        portInput.setLayoutParams(portInputParams);
        portRow.addView(portLabel);
        portRow.addView(portInput);
        buttonContainer.addView(portRow);
    ...
    private void startGame(String level) {
        ...
        int port = 7777;
        String portText = portInput != null ? portInput.getText().toString().trim() : "";
        if (!portText.isEmpty()) {
            try {
